...
switch ($op) {
	...
	xoops_cp_header();
	xpetitions_adminmenu(4, _AM_XPETITIONS_EMAILS);
	...
	if (!$update_emails) {
		redirect_header("email.php", 2, _AM_XPETITIONS_ERROR_UPDATE);
		// exit;
 	} else {
		redirect_header("email.php", 2, _AM_XPETITIONS_VALID_UPDATE);
	}
